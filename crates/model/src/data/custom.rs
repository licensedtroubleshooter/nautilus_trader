...
pub fn serialize_record_batch_to_ipc_bytes(
    ...
    let mut buffer = Vec::new();
    {
        let mut writer = arrow::ipc::writer::FileWriter::try_new(&mut buffer, &batch.schema())
            .map_err(|e| to_pyvalue_err(format!("Failed to create IPC writer: {e}")))?;
        writer
            .write(&batch)
            .map_err(|e| to_pyvalue_err(format!("Failed to write IPC batch: {e}")))?;
        writer
            .finish()
            .map_err(|e| to_pyvalue_err(format!("Failed to finish IPC writer: {e}")))?;
    }
