...
def customdataclass_pyo3(*args, **kwargs):  # noqa: C901 (too complex)
    ...
    def wrapper(cls):  # noqa: C901 (too complex)
        ...
        if "encode_record_batch_py" not in cls.__dict__:
            ...
            def encode_record_batch_py(self, items: list) -> bytes:
                ...
                batch = pa.RecordBatch.from_pylist(dicts, schema=self.__class__._schema)
                sink = pa.BufferOutputStream()
                # Use file format (with footer) so Rust FileReader can read it
        ...
        if "decode_record_batch_py" not in cls.__dict__:
            ...
            @classmethod
            def decode_record_batch_py(cls_inner, metadata: dict, ipc_bytes: bytes) -> list:
                # Use file format reader to match Rust FileWriter output
                reader = pa.ipc.open_file(BytesIO(ipc_bytes))
                table = reader.read_all()
                return [cls_inner.from_dict(d) for d in table.to_pylist()]
